<x-layout>
    <header
        class="px-8 py-8 bg-gradient-to-r from-green-500 to-teal-500 hover:from-green-600 hover:to-teal-600 rounded-xl sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 transition-all duration-300">
    ...
    <div x-data="orderDashboard" x-cloak class="p-4 sm:p-6 lg:p-8 font-sans">
        ...
        <div class="mb-8 px-6 py-6 rounded-2xl">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div
                    class="bg-white p-6 rounded-xl border border-gray-500 flex flex-col gap-4 transform hover:-translate-y-1 transition-transform duration-300">
                ...
                <div
                    class="bg-white p-6 border border-gray-500 rounded-xl flex flex-col gap-4 transform hover:-translate-y-1 transition-transform duration-300">
                    <div class="bg-green-100 p-3 rounded-full w-14 h-14 flex items-center justify-center">
                        <svg class="w-8 h-8 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                            xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 11.218 12 10.5 12 10.5s-1.536-.718-2.621-1.818c-1.172-.879-1.172-2.303 0-3.182C10.536 4.621 12 5.25 12 5.25s1.536.718 2.621 1.818l.879.659M7 12h10">
                            </path>
                        </svg>
                    </div>
                    <div>
                        <p class="text-sm text-gray-500">Total Pendapatan</p>
                        <p class="text-2xl font-bold text-gray-800" x-text="formatCurrency(todayStats.total_revenue)">
                        </p>
                    </div>
                </div>
                <div
            ...
            <div class="border-b border-gray-200">
                <nav class="-mb-px flex space-x-6 px-6" aria-label="Tabs">
                    <button @click="switchTab('active')"
                        ...
                        class="relative whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200 focus:outline-none">
                        Pesanan Aktif
                        <span x-show="newOrdersCount > 0" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="p-6 min-h-[400px]">
                ...
                <div x-show="activeTab === 'active'" x-transition:enter="transition ease-out duration-300"
                    ...
                    <div class="flex flex-col space-y-4">
                        <template x-for="order in activeOrders" :key="order.id">
                            <div
                                class="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow duration-300 flex flex-col md:flex-row md:items-center md:justify-between">
                                <div class="w-full md:w-48 flex-shrink-0 mb-4 md:mb-0 md:pr-4">
                                    ...
                                    <p class="text-sm text-gray-500">Meja: <span class="font-medium"
                                            x-text="order.table.name"></span></p>
                                </div>
                                <div
                                    class="flex-1 min-w-0 mb-4 md:mb-0 md:px-4 md:border-l md:border-r border-gray-200">
                                    <div class="flex items-center space-x-2 overflow-x-auto pb-2 scrollbar-thin">
                                        <template x-for="item in order.items" :key="item.id">
                                            <div
                                                class="flex-shrink-0 bg-gray-50 text-sm text-gray-700 rounded-full px-3 py-1 whitespace-nowrap">
                                ...
                                <div class="w-full md:w-64 flex-shrink-0 md:pl-4 flex items-center justify-between">
                                    ...
                                    <div class="flex items-center">
                                        <button @click="showOrderDetails(order.id)"
                                            class="text-gray-400 hover:text-indigo-600 p-2 rounded-full hover:bg-gray-100 transition-colors duration-200">
                                        ...
                                        <div class="relative inline-block text-left">
                                            <button @click="toggleActionMenu(order.id)" type="button"
                                                class="text-gray-400 hover:text-indigo-600 p-2 rounded-full hover:bg-gray-100 transition-colors duration-200">
                                            ...
                                            <div x-show="actionMenuOpenFor === order.id"
                                                @click.away="closeActionMenu()" x-transition
                                                class="origin-top-right absolute right-0 mt-2 w-48 rounded-xl shadow-lg bg-white ring-1 ring-black ring-opacity-5 z-10">
                                                <div class="py-1">
                                                    <template x-if="order.status === 'pending'">
                                                        <a href="#"
                                                            @click.prevent="updateStatus(order.id, 'paid')"
                                                    ...
                                                    <template x-if="order.status === 'paid'">
                                                        <a href="#"
                                                            ...
                                                            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Proses
                                                            Pesanan</a>
                                                    </template>
                                                    <template x-if="['paid', 'preparing'].includes(order.status)">
                                                        <a href="#"
                                                            @click.prevent="updateStatus(order.id, 'completed')"
                                                            class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Selesaikan</a>
                                                    </template>
                                                    <template
                                                        x-if="!['completed', 'cancelled'].includes(order.status)">
                                                        <div class="border-t border-gray-100 my-1"></div>
                                                        <a href="#"
                                                            @click.prevent="updateStatus(order.id, 'cancelled')"
                                                            class="block px-4 py-2 text-sm text-red-600 hover:bg-red-50">Batalkan</a>
                                                    </template>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <!-- Empty State -->
                        <div x-show="activeOrders.length === 0"
                            class="col-span-full text-center py-16 text-gray-500 flex flex-col items-center justify-center">
                    ...
                    <div x-show="activePagination.last_page > 1"
                        class="mt-6 flex items-center justify-between border-t border-gray-200 pt-4">
                        <div class="flex-1 flex justify-between sm:hidden">
                            <button @click="changeActivePage(activePagination.current_page - 1)"
                        ...
                        <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                            <div>
                                <p class="text-sm text-gray-700">
                                    Menampilkan <span class="font-medium" x-text="activePagination.from"></span>
                                    sampai <span class="font-medium" x-text="activePagination.to"></span> dari <span
                                        class="font-medium" x-text="activePagination.total"></span> pesanan
                                </p>
                            </div>
                            <div>
                                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                                    ...
                                    <template
                                        x-for="page in getPageNumbers(activePagination.current_page, activePagination.last_page)"
                                        :key="page">
                                        <button @click="page !== '...' && changeActivePage(page)"
                                            :class="{
                                                'bg-indigo-50 border-indigo-500 text-indigo-600 z-10': page ===
                                                    activePagination
                                                    .current_page,
                ...
                <div x-show="activeTab === 'history'" x-transition:enter="transition ease-out duration-300"
                        ...
                        <div class="flex flex-col lg:flex-row lg:items-end gap-4">
                            <div class="flex-shrink-0">
                                ...
                                <div class="flex items-center rounded-lg p-1 space-x-1">
                                    <button @click="setFilterRange('today')"
                                        :class="{ 'bg-indigo-600 text-white shadow': activePreset === 'today', 'hover:bg-gray-200 text-gray-600': activePreset !== 'today' }"
                                        class="px-3 py-1.5 text-sm font-medium rounded-md transition-all">Hari
                                        Ini</button>
                                    <button @click="setFilterRange('last7days')"
                                        :class="{ 'bg-indigo-600 text-white shadow': activePreset === 'last7days', 'hover:bg-gray-200 text-gray-600': activePreset !== 'last7days' }"
                                        class="px-3 py-1.5 text-sm font-medium rounded-md transition-all">7 Hari
                                        Terakhir</button>
                                    <button @click="setFilterRange('thismonth')"
                                        :class="{ 'bg-indigo-600 text-white shadow': activePreset === 'thismonth', 'hover:bg-gray-200 text-gray-600': activePreset !== 'thismonth' }"
                                        class="px-3 py-1.5 text-sm font-medium rounded-md transition-all">Bulan
                                        Ini</button>
                                </div>
                            </div>
                            ...
                            <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label for="startDate" class="text-sm font-medium text-gray-700 mb-2 block">Dari
                                        Tanggal</label>
                                    <div class="relative">
                                        <div
                                        ...
                                        <input x-ref="startDate" type="text" id="startDate"
                                            class="w-full pl-10 pr-3 py-2 rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition"
                                            placeholder="YYYY-MM-DD">
                                    </div>
                                </div>
                                <div>
                                    <label for="endDate" class="text-sm font-medium text-gray-700 mb-2 block">Sampai
                                        Tanggal</label>
                                    <div class="relative">
                                        <div
                                            class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                        ...
                                        <input x-ref="endDate" type="text" id="endDate"
                                            class="w-full pl-10 pr-3 py-2 rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition"
                                            placeholder="YYYY-MM-DD">
                                    </div>
                                </div>
                            ...
                            <div class="flex-shrink-0">
                                <button @click="fetchOrderHistory"
                                    class="w-full bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-lg font-medium transition-all duration-200 shadow-sm hover:shadow-md disabled:opacity-50 disabled:cursor-wait flex items-center justify-center space-x-2"
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                                        ...
                                        <template x-for="report in orderHistory" :key="report.id">
                                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"
                                                    x-text="report.table.name"></td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"
                                                    x-text="formatDateTime(report.created_at)"></td>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <span :class="getStatusClass(report.status)"
                                                        class="text-xs font-semibold px-3 py-1.5 rounded-full"
                                                        x-text="report.status.charAt(0).toUpperCase() + report.status.slice(1)"></span>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium text-gray-800"
                                                    x-text="formatCurrency(report.total_amount)"></td>
                                            </tr>
                                        </template>
                                    </template>
                                    <tr x-show="!isLoading.history && orderHistory.length === 0">
                                        <td colspan="5" class="text-center py-16 text-gray-500">
                                            <div class="flex flex-col items-center">
                                                <div
                                                    class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-full p-4 mb-4">
                                                    <svg class="h-8 w-8 text-gray-400" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            stroke-width="2"
                                                            d="M9 12h6m-6-4h6m2 5l-3-3 3-3m-5 14H5a2 2 0 01-2-2V5a2 2 0 012-2h14a2 2 0 012 2v14a2 2 0 01-2 2z" />
                                                    </svg>
                                                </div>
                                                <p class="text-sm font-medium">Tidak ada data riwayat</p>
                                                <p class="text-xs text-gray-400 mt-1">untuk rentang tanggal yang
                                                    dipilih</p>
                                            </div>
                                        </td>
                                    </tr>
                        ...
                        <div x-show="historyPagination.last_page > 1"
                            class="bg-white px-4 py-3 border-t border-gray-200 sm:px-6">
                            <div class="flex items-center justify-between">
                                <div class="flex-1 flex justify-between sm:hidden">
                                    <button @click="changeHistoryPage(historyPagination.current_page - 1)"
                                    ...
                                    <div>
                                        <p class="text-sm text-gray-700">
                                            Menampilkan <span class="font-medium"
                                                x-text="historyPagination.from"></span> sampai <span
                                                class="font-medium" x-text="historyPagination.to"></span> dari <span
                                                class="font-medium" x-text="historyPagination.total"></span> riwayat
                                        </p>
                                    </div>
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                                            ...
                                            <template
                                                x-for="page in getPageNumbers(historyPagination.current_page, historyPagination.last_page)"
                                                :key="page">
                                                <button @click="page !== '...' && changeHistoryPage(page)"
                                                    :class="{
                                                        'bg-indigo-50 border-indigo-500 text-indigo-600': page ===
                                                            historyPagination
                                                            .current_page,
                                                        'bg-white border-gray-300 text-gray-500 hover:bg-gray-50': page !==
        ...
        <div x-show="modal.isOpen"
            ...
            <div @click.away="closeModal" x-show="modal.isOpen" x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 transform scale-90"
                x-transition:enter-end="opacity-100 transform scale-100"
                x-transition:leave="transition ease-in duration-200"
                x-transition:leave-start="opacity-100 transform scale-100"
                x-transition:leave-end="opacity-0 transform scale-90"
                class="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-auto">
                <div class="p-6">
                    <div class="flex justify-between items-center border-b border-gray-100 pb-4 mb-6">
                        <h3 class="text-xl font-bold text-gray-800">Detail Pesanan #<span
                                x-text="modal.data?.id"></span></h3>
                    ...
                    <div>
                        <template x-if="isLoading.modal">
                            <div class="text-center py-12">
                                <svg class="animate-spin mx-auto h-8 w-8 text-indigo-600 mb-4"
                                    xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10"
                                        stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor"
                        ...
                        <template x-if="!isLoading.modal && modal.data">
                            <div class="space-y-6">
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-3">
                                        ...
                                        <p class="font-semibold text-gray-800"
                                            x-text="formatDateTime(modal.data.created_at)"></p>
                                    </div>
                                    <div class="bg-gray-50 rounded-lg p-3">
                                        <p class="text-xs text-gray-500 mb-1">Status</p>
                                        <p>
                                            <span :class="getStatusClass(modal.data.status)"
                                                class="text-xs font-semibold px-3 py-1.5 rounded-full"
                                                x-text="modal.data.status.charAt(0).toUpperCase() + modal.data.status.slice(1)"></span>
                                        </p>
                                    </div>
                                </div>
                                <div class="border-t border-gray-100 pt-4">
                                    ...
                                    <div
                                        ...
                                        <template x-for="item in modal.data.items" :key="item.id">
                                            <div
                                                class="flex justify-between items-center bg-white border rounded-lg p-3">
                                                <div>
                                                    <p class="font-medium text-gray-900" x-text="item.product.name">
                                                    </p>
                                ...
                                <div
                                    class="border-t border-gray-100 pt-4 flex justify-between items-center bg-gray-50 rounded-lg p-4">
                                    <p class="text-lg font-bold text-gray-800">Total Keseluruhan</p>
                                    <p class="text-2xl font-bold text-indigo-600"
                                        x-text="formatCurrency(modal.data.total_amount)"></p>
                                </div>
                            </div>
                        </template>
                    </div>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('orderDashboard', () => ({
                ...
                async changeActivePage(page) {
                    if (page < 1 || page > this.activePagination.last_page || this.isLoading.active)
                        return;

                    this.isLoading.active = true;
                    try {
                        const response = await fetch(
                            `{{ route('admin.orders.active') }}?page=${page}`);
                ...
                async changeHistoryPage(page) {
                    if (page < 1 || page > this.historyPagination.last_page || this.isLoading
                        .history) return;
                    ...
                    try {
                        ...
                        const response = await fetch(
                            `{{ route('admin.orders.history') }}?${params}`);
                        const data = await response.json();
                ...
                async updateStatus(orderId, newStatus) {
                    ...
                    try {
                        ...
                        const activeIndex = this.activeOrders.findIndex(o => o.id === updatedOrder
                            .id);
