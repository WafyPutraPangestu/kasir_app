    ...
    <div x-data="orderDashboard" x-cloak class="container mx-auto p-4 sm:p-6 lg:p-8 font-sans">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100">
            ...
            <div class="p-6 min-h-[400px]">
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        <div class="relative inline-block text-left">
                                            ...
                                            <div x-show="actionMenuOpenFor === order.id" @click.away="closeActionMenu()" x-transition class="origin-top-right absolute right-0 mt-2 w-48 rounded-xl shadow-lg bg-white ring-1 ring-black ring-opacity-5 z-10">
                                                <div class="py-1">
                                                    <template x-if="order.status === 'pending'"><a href="#" @click.prevent="updateStatus(order.id, 'paid')" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Tandai Bayar</a></template>
                                                    <template x-if="order.status === 'paid'"><a href="#" @click.prevent="updateStatus(order.id, 'preparing')" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Proses Pesanan</a></template>
                                                    <template x-if="['paid', 'preparing'].includes(order.status)"><a href="#" @click.prevent="updateStatus(order.id, 'completed')" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Selesaikan</a></template>
                ...
                {{-- [PERUBAHAN] Panel Riwayat Pesanan dengan Filter Baru --}}
                    ...
                    <div class="mb-6 p-4 bg-white border border-gray-200 rounded-xl shadow-sm">
                        <div class="flex flex-col lg:flex-row lg:items-end gap-4">
                            ...
                            <div class="flex-shrink-0">
                                ...
                                <div class="flex items-center border border-gray-200 rounded-lg p-1 bg-gray-50 space-x-1">
                                ...
                                <tbody class="bg-white divide-y divide-gray-100">
                                    <template x-if="isLoading.history">
                                        <template x-for="i in 5">
                                            <tr><td class="px-6 py-4" colspan="5"><div class="animate-pulse flex space-x-4"><div class="rounded-full bg-gray-200 h-4 w-4"></div><div class="flex-1 space-y-2"><div class="h-4 bg-gray-200 rounded w-3/4"></div><div class="h-4 bg-gray-200 rounded w-1/2"></div></div></div></td></tr>
                                        </template>
                                    </template>
                                    <template x-if="!isLoading.history">
                                        <template x-for="report in orderHistory" :key="report.id">
                                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap"><span :class="getStatusClass(report.status)" class="text-xs font-semibold px-3 py-1.5 rounded-full" x-text="report.status.charAt(0).toUpperCase() + report.status.slice(1)"></span></td>
                                        ...
                                        <td colspan="5" class="text-center py-16 text-gray-500">
                                            <div class="flex flex-col items-center">
                                                <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-full p-4 mb-4">
                                                    <svg class="h-8 w-8 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6-4h6m2 5l-3-3 3-3m-5 14H5a2 2 0 01-2-2V5a2 2 0 012-2h14a2 2 0 012 2v14a2 2 0 01-2 2z" /></svg>
                                                </div>
                 ...
                 <div class="p-6">
                    ...
                    <div>
                        <template x-if="isLoading.modal">
                            <div class="text-center py-12"><svg class="animate-spin mx-auto h-8 w-8 text-indigo-600 mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg><p class="text-gray-500 font-medium">Memuat data...</p></div>
                        </template>
                        <template x-if="!isLoading.modal && modal.data">
                            <div class="space-y-6">
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    <div class="bg-gray-50 rounded-lg p-3"><p class="text-xs text-gray-500 mb-1">Meja</p><p class="font-semibold text-gray-800" x-text="modal.data.table.name"></p></div>
                                    <div class="bg-gray-50 rounded-lg p-3"><p class="text-xs text-gray-500 mb-1">Tanggal</p><p class="font-semibold text-gray-800" x-text="formatDateTime(modal.data.created_at)"></p></div>
                                    <div class="bg-gray-50 rounded-lg p-3"><p class="text-xs text-gray-500 mb-1">Status</p><p><span :class="getStatusClass(modal.data.status)" class="text-xs font-semibold px-3 py-1.5 rounded-full" x-text="modal.data.status.charAt(0).toUpperCase() + modal.data.status.slice(1)"></span></p></div>
                                </div>
                                        ...
                                        <template x-for="item in modal.data.items" :key="item.id">
                                            <div class="flex justify-between items-center bg-white border rounded-lg p-3">
                                                <div>
                                                    ...
                                                    <p class="text-sm text-gray-500"><span x-text="item.quantity"></span> x <span x-text="formatCurrency(item.price)"></span></p>
                                                </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('orderDashboard', () => ({
                ...
                filter: { startDate: @json($startDate), endDate: @json($endDate) },
                ...
                toggleActionMenu(orderId) { this.actionMenuOpenFor = this.actionMenuOpenFor === orderId ? null : orderId; },
                closeActionMenu() { this.actionMenuOpenFor = null; },
                ...
                async fetchOrderHistory() {
                    if (this.isLoading.history) return; this.isLoading.history = true;
                    try {
                    ...
                    } catch (error) { this.showToast(error.message || 'Gagal memuat riwayat', 'error'); } 
                    finally { this.isLoading.history = false; }
                },
                ...
                async showOrderDetails(orderId) {
                    this.modal.isOpen = true; this.isLoading.modal = true; this.modal.data = { id: orderId };
                    try {
                        const response = await fetch(`/admin/orders/${orderId}`);
                        const data = await response.json();
                        if (data.success) { this.modal.data = data.order; } 
                        else { throw new Error(data.message); }
                    } catch (error) { this.showToast(error.message || 'Gagal memuat detail', 'error'); this.closeModal(); } 
                ...
                async updateStatus(orderId, newStatus) {
                    ...
                    try {
                        ...
                        if (['completed', 'cancelled'].includes(updatedOrder.status)) {
                            ...
                            if (!this.orderHistory.some(r => r.id === updatedOrder.id)) this.orderHistory.unshift(updatedOrder);
                        } else {
                            if (activeIndex !== -1) this.activeOrders[activeIndex] = updatedOrder;
                        }

                    } catch (error) { this.showToast(error.message || `Gagal mengubah status`, 'error'); }
                },
                
                showToast(message, type = 'info') {
                    Toastify({
                        text: message, duration: 3000, close: true, gravity: "top", position: "right",
                        style: { background: {success: '#10B981', error: '#EF4444', info: '#3B82F6'}[type] || '#6B7280' }
                    }).showToast();
                }
            }));
        });
