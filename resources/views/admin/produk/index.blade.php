                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            <div class="p-2 bg-indigo-100 rounded-lg">
                                <svg class="w-5 sm:w-6 h-5 sm:h-6 text-indigo-600" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="">
                            <tr>
                                ...
                                <th scope="col"
                                    class="hidden md:table-cell px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                    Kategori
                                </th>
                                <th scope="col"
                                    class="hidden lg:table-cell px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                    Harga
                                </th>
                                <th scope="col"
                                    class="hidden lg:table-cell px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                            ...
                            @forelse ($products as $product)
                                <tr class="hover:bg-white transition-colors duration-150">
                                    ...
                                    <td class="hidden md:table-cell px-4 sm:px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-xs sm:text-sm font-medium bg-gray-100 text-gray-800">
                                            {{ $product->category->name }}
                                        </span>
                                    </td>
                                    <td class="hidden lg:table-cell px-4 sm:px-6 py-4 whitespace-nowrap">
                                        <span
                                            class="inline-flex items-center px-3 py-1 rounded-full text-xs sm:text-sm font-medium text-gray-800">
                                    ...
                                    <td class="hidden lg:table-cell px-4 sm:px-6 py-4 whitespace-nowrap">
                                        <span
                                            ...
                                            {{ $product->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                            <svg class="w-3 sm:w-4 h-3 sm:h-4 mr-1" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="{{ $product->is_active ? 'M5 13l4 4L19 7' : 'M6 18L18 6M6 6l12 12' }}">
                                                </path>
                                            </svg>
                                            {{ $product->is_active ? 'Aktif' : 'Nonaktif' }}
                                        </span>
                                    ...
                                    <td class="px-4 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        ...
                                        <form id="delete-form-{{ $product->id }}"
                                            action="{{ route('admin.produk.destroy', $product->id) }}" method="POST"
                                            class="hidden">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                ...
                                <tr>
                                    <td colspan="6" class="px-4 sm:px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            <svg class="w-12 sm:w-16 h-12 sm:h-16 text-gray-300 mb-4" fill="none"
                                                stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                    d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4">
                                                </path>
                                            </svg>
                                            <h3 class="text-base sm:text-lg font-medium text-gray-900 mb-2">Belum Ada
                                                Produk</h3>
                                            <p class="text-xs sm:text-sm text-gray-500 mb-4">Mulai dengan menambahkan
                                                produk pertama Anda</p>
                                            <a href="{{ route('admin.produk.create') }}"
    ...
    <div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 hidden">
        <div class="relative top-20 mx-auto p-4 sm:p-5 border w-80 sm:w-96 shadow-lg rounded-2xl bg-white">
            <div class="mt-3 text-center">
                <div
                    class="mx-auto flex items-center justify-center h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-red-100 mb-4">
    ...
    @push('scripts')
        <script>
            ...
            function showToast(message, type = 'success') {
                ...
                toast.id = toastId;
                toast.className =
                    `${bgColor} text-white px-4 sm:px-6 py-3 sm:py-4 rounded-lg shadow-lg transform translate-x-full transition-transform duration-300 ease-in-out max-w-xs sm:max-w-sm`;
                toast.innerHTML = `
                <div class="flex items-center justify-between">
